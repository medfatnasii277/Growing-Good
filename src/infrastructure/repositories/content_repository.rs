use crate::domain::{
    Category, ContentItem, ContentType, CreateCategoryRequest, CreateContentRequest,
...
impl ContentRepository {
    ...
    pub fn find_all_categories(&self) -> Result<Vec<Category>, ContentError> {
        ...
        let mut stmt =
            conn.prepare("SELECT id, name, description, created_at FROM categories ORDER BY id")?;
    ...
    pub fn create(&self, request: CreateContentRequest) -> Result<ContentItem, ContentError> {
        ...
        let data_json = serde_json::to_string(&request.data).map_err(|e| {
            ContentError::Database(rusqlite::Error::ToSqlConversionFailure(Box::new(e)))
        })?;

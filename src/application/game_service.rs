use crate::domain::{
    CompleteContentRequest, ContentType, UserProgress, ValidateAnswerRequest,
    ValidateAnswerResponse,
};
use crate::infrastructure::repositories::progress_repository::LeaderboardEntry;
use crate::infrastructure::{
    ContentError, ContentRepository, Database, ProgressError, ProgressRepository,
};
...
impl GameService {
    ...
    ) -> Result<ValidateAnswerResponse, GameError> {
            ...
            for (idx, ans) in answers.iter().enumerate() {
                if let (Some(question_obj), Some(selected_idx)) = (
                    ...
                    if selected_idx == correct_idx {
                        ...
                        let question_text = question_obj
                            .get("question")
                            .and_then(|v| v.as_str())
                            .unwrap_or("Question");
                        feedback_parts.push(format!(
                            "Question {}: Not quite. The correct answer was: {}",
                            idx + 1,
                            question_text
                        ));
                    }
    ...
    fn validate_reading(
        ...
        if completed {
            Ok(ValidateAnswerResponse {
                ...
                points: 100,
                feedback: format!(
                    "Great job! You read the story ({} min). Remember: {}",
                    reading_time,
                    data.get("moral")
                        .and_then(|v| v.as_str())
                        .unwrap_or("Always be kind!")
                ),
            })
        } else {
            Ok(ValidateAnswerResponse {
    ...
    fn validate_click_game(
        ...
        let feedback = if all_correct {
            format!(
                "Excellent! You made the right choices in: {}. Always remember to stay safe! 🛡️",
                scenario
            )
        } else if wrong_clicks > 0 {
            format!(
                "Be careful! Some of your choices were unsafe. Remember: {}",
                data.get("safety_tip")
                    .and_then(|v| v.as_str())
                    .unwrap_or("Stay away from danger!")
            )
        } else {
            format!(
                "You missed some safe choices! Look for: {}",
                correct_items.join(", ")
            )
        };
    ...
    pub fn get_progress(
        ...
        user_id: i64,
        content_id: i64,
    ) -> Result<Option<UserProgress>, GameError> {
        let repo = ProgressRepository::new(&self.db);
    ...
    pub fn get_leaderboard(&self, limit: i64) -> Result<Vec<LeaderboardEntry>, GameError> {
        let repo = ProgressRepository::new(&self.db);
        Ok(repo.get_leaderboard(limit)?)
    }

    pub fn get_user_rank(&self, user_id: i64) -> Result<i64, GameError> {
        let repo = ProgressRepository::new(&self.db);
        Ok(repo.get_user_rank(user_id)?)
    }
}
